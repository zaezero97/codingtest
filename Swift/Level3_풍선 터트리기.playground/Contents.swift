...
// 따라서 풍선을 기준으로 왼쪽 부분 오른쪽 부분의 최솟값중 더 큰값보다 작기만 하면 된다. why? 일단 큰값보다 작아야 하는 이유 작은값보다 만약크다하면 큰값보단 작을 수 있으니 큰 값과 비교해야한다. 그리고 작은 값의 부분의 최솟값은 그 숫자보다 작거나 같기 때문에 큰 값보다 반대쪽부분의 최솟값이 작을 것이 확정이기 때문이다.
